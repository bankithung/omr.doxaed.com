...
const OUT = join(dirname(fileURLToPath(import.meta.url)), "screenshots", "students")
...
await section("Section A", [["101", "Asha Devi"], ["102", "Ravi Kumar"], ["103", "Meera Nair"]])
...
await p.goto(`${F}/classes/${cls.id}/students`, { waitUntil: "load" })
await p.waitForTimeout(1600)
console.log("overlay:", await p.locator("vite-error-overlay").count())
await p.screenshot({ path: `${OUT}/01-students.png` })
await p.getByRole("button", { name: "Add student" }).first().click()
await p.waitForTimeout(600)
await p.screenshot({ path: `${OUT}/02-add.png` })
await ctx.close()
await b.close()
console.log("DONE")
