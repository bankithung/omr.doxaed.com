...
const OUT = join(dirname(fileURLToPath(import.meta.url)), "screenshots", "exam-workspace")
...
const roster = await (await fetch(`${API}/rosters/`, { method: "POST", headers: O, body: JSON.stringify({ name: "Students", class_group: secA.id }) })).json()
await fetch(`${API}/students/`, { method: "POST", headers: O, body: JSON.stringify({ roster: roster.id, roll_number: "101", full_name: "Asha Devi" }) })
const test = await (await fetch(`${API}/tests/`, { method: "POST", headers: O, body: JSON.stringify({ class_group: secA.id, title: "Section A Quiz", subject: "Science", mode: "standard" }) })).json()
...
console.log("test", test.id, "class", cls.id, "secA", secA.id)
...
await p.screenshot({ path: `${OUT}/01-generate.png` })

await p.goto(`${F}/tests/${test.id}/results`, { waitUntil: "load" })
await p.waitForTimeout(2600)
await p.screenshot({ path: `${OUT}/02-results.png` })

await ctx.close(); await b.close()
console.log("DONE — expect lifecycle nav (Build/Generate/Scan/Review/Results/Analytics), Generate active, Back to class")
