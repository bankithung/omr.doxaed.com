...
urlpatterns = [
    ...
    path("omr/scan-batches/<uuid:pk>/", views.ScanBatchDetailView.as_view(), name="omr-scan-batch-detail"),
    path(
        "omr/scan-batches/<uuid:pk>/sheets/",
        views.ScanBatchSheetsView.as_view(),
        name="omr-scan-batch-sheets",
    ),
    path(
        "omr/scan-jobs/<uuid:pk>/warped/",
